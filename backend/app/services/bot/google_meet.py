...
async def join_meeting_auto(meeting_url: str):
    ...
    log("=" * 60)
    log("Automated Google Meet Join Bot")
    log("=" * 60)
    log(f"Meeting: {meeting_url}")
    log("=" * 60)
    ...
    try:
        ...
        if 'accounts.google.com' in current_url and 'signin' in current_url:
            ...
            if 'meet.google.com' not in current_url:
                log("[WARN] Still not on meeting page after 60s")
                log("[INFO] Browser will stay open - you can continue manually")
                
                # Stay open indefinitely on error
                try:
                    while True:
                         await page.wait_for_timeout(60000)
        ...
        if 'meet.google.com' in current_url:
            ...
            # STEP 3: Enter display name (if Guest)
            ...
            try:
                ...
                if clicked:
                    ...
                    # STEP 7: Confirm entry
                    print("[INFO] Verifying entry...")
                    log("Verifying entry by waiting for meeting UI...")
                    
                    # Wait for elements that indicate we are INSIDE the meeting
                    # e.g. "Meeting details", "Leave call", or specific buttons
                    try:
                        await page.wait_for_selector(
                            'button[aria-label*="Leave call"], button[aria-label*="End call"], div[data-meeting-title]', 
                            timeout=20000
                        )
                        print("[SUCCESS] confirmed entry into meeting!")
                        log("CONFIRMED: Successfully joined the meeting")
                    except:
                        print("[WARN] Could not definitively confirm entry (might be waiting for approval)")
                        log("WARN: Navigation complete but entry confirmation timed out")
            ...
            # If we exited the loop or finished without clicking
            # Keep open
            log("\n[INFO] Process finished. Keeping browser open for manual inspection.")
            try:
                while True:
                    await page.wait_for_timeout(60000)
            except KeyboardInterrupt:
        ...
        try:
             ...
             # Try to keep open even on critical error if possible
             if 'page' in locals():
                 print("[INFO] Keeping browser open after error...")
                 while True:
                     # Synchronous sleep as async might be broken
                     import time
                     time.sleep(60)
        except:
             pass
    ...
    # Should not reach here usually due to infinite loops
    return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Default test URL (Google Meet dummy)
